...
namespace Exercise1
{
	internal static class Architecture
	{
		...
		public static void Architect(string path)
		{
			...
			FileInfo AUTHORS = new FileInfo(path);
			AUTHORS.Create();
			File.AppendAllText(path,"* etienne.lazarz\n");
			FileInfo README = new FileInfo(path);
			README.Create();
			string phrase = "Everything in programming is magic... except for the programmer\n";
			File.AppendAllText(path, phrase);
			Directory.CreateDirectory(Path.Combine(path,"TP1")); // wrote both methode to create the dir
			FileInfo useless = new FileInfo(path + "TP1/");
			useless.Create();
		}
	}
}